...
## Features
Below I will highlight some of the traits of the custom effects including:
...
The picture below compares the scene with only basic post effects (top) with the Kuwahara filter (middle) and the custom effects (bottom). The Kuwahara filter is able to reduce noise, but they fail to preserve the base forms of the rock. When the Kuwahara filter is replaced by the custom effects, the forms of the rock are preserved while the surface detail is simplified. 
...
The picture below compares the scene with only basic post effects (left) with the custom effects (right). The scene on the left shows visible seams between the branches and the trunk of the tree. Since the two parts are similar in color and value, the custom effects are able to blend and hide these seams.
